...
import "package:smart_expense/features/home/application/home_view_model.dart";

void main() {
  const pager = HomeHistoryPager();
  ...
  test("reset clears loaded history and keeps pagination open", () {
    final viewModel = const HomeViewModel.initial().copyWith(
      history: [tx("1")],
      historyOffset: 1,
      allLoaded: true,
    );
  ...
  test(
    ...
    () {
      final next = pager.appendBatch(const HomeViewModel.initial(), [
        tx("1"),
        tx("2"),
      ], pageSize: 2);
  ...
  test("appendBatch marks allLoaded when returned page is empty", () {
    final next = pager.appendBatch(
      const HomeViewModel.initial().copyWith(loadingMore: true),
      const [],
      pageSize: 2,
    );
