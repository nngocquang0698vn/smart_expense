...
import "package:smart_expense/core/widgets/app_date_range_picker.dart";
...
void main() {
  ...
  testWidgets("shows not-selected subtitle when range empty", (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: AppDateRangePicker(onChanged: (_) {}),
        ),
      ),
    );

    expect(find.text(AppStrings.dateRangeNotSelected), findsOneWidget);
  });
  ...
  testWidgets("clear button calls onChanged with null", (tester) async {
    ...
    await tester.tap(find.text(AppStrings.dateRangeClear));
    await tester.pump();
